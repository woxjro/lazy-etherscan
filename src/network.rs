...
impl<'a> Network<'a> {
    ...
    pub async fn handle_network_event(&mut self, io_event: IoEvent) {
        match io_event {
            IoEvent::GetLatestBlocks => {
                ...
                let blocks = Self::get_latest_blocks(self.endpoint).await.unwrap();
                app.latest_blocks = Some(StatefulList::with_items(blocks));
                app.is_loading = false;
            }
            IoEvent::GetLatestTransactions => {
                let mut app = self.app.lock().await;
                let transactions = Self::get_latest_transactions(self.endpoint).await.unwrap();
    ...
    async fn get_latest_blocks(endpoint: &'a str) -> Result<Vec<Block<H256>>, Box<dyn Error>> {
